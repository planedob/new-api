...
func CacheGetRandomSatisfiedChannel(param *RetryParam) (*model.Channel, string, error) {
	...
	if param.TokenGroup == "auto" {
		...
		autoGroups, autoErr := GetUserSelectableAutoGroups(param.Ctx.GetInt("id"), userGroup)
		if autoErr != nil {
			return nil, selectGroup, autoErr
		}
		...
		for i := startGroupIndex; i < len(autoGroups); i++ {
			...
			channel, _ = model.GetRandomSatisfiedChannelWithPolicy(autoGroup, param.ModelName, priorityRetry, param.StopAtExhaustion)
			if channel == nil {
		...
		channel, err = model.GetRandomSatisfiedChannelWithPolicy(param.TokenGroup, param.ModelName, param.GetRetry(), param.StopAtExhaustion)
		if err != nil {
...
func cacheGetRandomSatisfiedChannelExhaustive(param *RetryParam) (*model.Channel, string, error) {
	...
	autoGroups, autoErr := GetUserSelectableAutoGroups(param.Ctx.GetInt("id"), userGroup)
	if autoErr != nil {
		return nil, param.TokenGroup, autoErr
	}
	startGroupIndex := 0
	if lastGroupIndex, exists := common.GetContextKey(param.Ctx, constant.ContextKeyAutoGroupIndex); exists {
		if idx, ok := lastGroupIndex.(int); ok {
