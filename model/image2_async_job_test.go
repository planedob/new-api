...
func TestCountActiveImage2AsyncJobsExcludesTerminalRows(t *testing.T) {
	prefix := setupImage2AsyncJobTestDB(t)
	active := image2AsyncTestJob(prefix, "active-count")
	terminal := image2AsyncTestJob(prefix, "terminal-count")
	terminal.Status = Image2AsyncJobStatusSucceeded
	now := time.Now().UTC()
	terminal.FinishedAt = &now
	require.NoError(t, CreateImage2AsyncJob(active))
	require.NoError(t, CreateImage2AsyncJob(terminal))

	count, err := CountActiveImage2AsyncJobs(now)
	require.NoError(t, err)
