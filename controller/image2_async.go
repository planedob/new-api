...
func SubmitImage2Job(c *gin.Context) {
	operation := image2AsyncOperation(c)
	...
	idempotencyKey := image2AsyncRequestIdempotencyKey(c)
...
func GetImage2Job(c *gin.Context) {
	...
	payload["poll_url"] = image2AsyncPollURL(c, snapshot.ID)
...
func writeImage2AsyncJobAccepted(c *gin.Context, snapshot image2AsyncJobSnapshot) {
	...
	c.JSON(status, gin.H{
		...
		"request_id": snapshot.RequestID,
		"status":     snapshot.Status,
		"poll_url":   image2AsyncPollURL(c, snapshot.ID),
		"created_at": snapshot.CreatedAt,
	})
}

func image2AsyncOperation(c *gin.Context) string {
	operation := strings.ToLower(strings.TrimSpace(c.Param("operation")))
	if operation == "" && strings.HasPrefix(c.Request.URL.Path, "/v1/images/generations/jobs") {
		return "generations"
	}
	return operation
}

func image2AsyncRequestIdempotencyKey(c *gin.Context) string {
	key := strings.TrimSpace(c.GetHeader("X-Image2-Idempotency-Key"))
	if key == "" {
...
func image2AsyncPollURL(c *gin.Context, id string) string {
	if strings.HasPrefix(c.Request.URL.Path, "/v1/images/generations/jobs") {
		return "/v1/images/generations/jobs/" + id
	}
	return "/pg/images/jobs/" + id
}
