...
func TestImage2AsyncPublicGenerationPathDefaultsOperationAndPollURL(t *testing.T) {
	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/v1/images/generations/jobs", nil)
	ctx.Params = nil
	require.Equal(t, "generations", image2AsyncOperation(ctx))
	require.Equal(t, "/v1/images/generations/jobs/job-1", image2AsyncPollURL(ctx, "job-1"))
}

func TestImage2AsyncAcceptsStandardIdempotencyHeader(t *testing.T) {
	recorder := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(recorder)
	ctx.Request = httptest.NewRequest(http.MethodPost, "/v1/images/generations/jobs", nil)
	ctx.Request.Header.Set("Idempotency-Key", "standard-key")
	ctx.Params = nil
	require.Equal(t, "standard-key", image2AsyncRequestIdempotencyKey(ctx))
}
