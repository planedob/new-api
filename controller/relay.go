...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	relayErrorStage = "image2_capability_validation"
...
func shouldRetry(c *gin.Context, info *relaycommon.RelayInfo, openaiErr *types.NewAPIError, retryTimes int, attemptElapsed time.Duration) bool {
	...
	if common.SafeFailoverV1Enabled || scopedGPTTestFailover || c.GetBool("image2_smart_router_active") {
		...
		decision := service.EvaluateSafeFailover(failoverInput)
		if c.GetBool("image2_smart_router_active") {
			decision = service.EvaluateImage2SafeFailover(failoverInput)
		}
...
func RelayMidjourney(c *gin.Context) {
	...
	if mjErr != nil {
		...
		logger.LogError(c, fmt.Sprintf("relay error (channel #%d, status code %d): %s", channelId, statusCode, service.SanitizeRelayErrorLogText(fmt.Sprintf("%s %s", mjErr.Description, mjErr.Result))))
	}
}
